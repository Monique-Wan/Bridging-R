...
# Instead of using package car, let's generate some random numbers and a linear model
x = runif(100,min=-5,max=5)
u = rnorm(100,mean=0,sd = 1)
y = 2*x+u
data = data.frame(reg = x, dv =y)
colnames(data)
plot(data$reg,data$dv)
abline(lm(data$dv ~ data$reg))

plot(data$reg,data$dv, 
     main="Relationship of dv and reg",
     ylab="dv", xlab="reg",
     pch=16, col="red") 
abline(lm(data$dv ~ data$reg), lty=2)
